...
- **Revenue by Zone and market_name** — a treemap where rectangle area is proportional to revenue, making the geographic imbalance impossible to miss at a glance. North's block occupies more than half the entire chart, and within it Delhi NCR's 470.76M tile dwarfs every other market in the business. Central's three main markets (Mumbai 135.26M, Nagpur 51.05M, Bhopal1 37.7M) are legible but clearly secondary. South is reduced to a thin column of truncated labels on the right edge — an accurate visual representation of its revenue contribution relative to the rest of the business.
- **Highest Value Market** — a KPI card showing Delhi NCR at 52.72% revenue share in the unfiltered state. Applying the Filter By Zone slicer shifts this to the highest-revenue market within the selected zone, making the card reusable across geographic scopes.
...
- The underlying dataset is the AtliQ Hardware sales dump from Code Basics. The RFM segmentation, hypothesis testing, and geographic drill-down are extensions beyond the tutorial scope.
